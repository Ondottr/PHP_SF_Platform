    ...
    public function regenerateEntities( string $classOrNamespace ): void
    {
        ...
        elseif ( class_exists( $classOrNamespace ) ) {
            throw new RuntimeCommandException(
                sprintf( 'Could not find Doctrine metadata for "%s". Is it mapped as an entity?', $classOrNamespace )
            );
        }
        elseif ( empty( $metadata ) ) {
            throw new RuntimeCommandException(
                sprintf( 'No entities were found in the "%s" namespace.', $classOrNamespace )
            );
        }
        ...
        foreach ( $metadata as $classMetadata ) {
            ...
            foreach ( $classMetadata->embeddedClasses as $fieldName => $mapping ) {
                ...
                $operations[ $embeddedClasses[ $fieldName ] ] = $this->createClassManipulator(
                    $embeddedClasses[ $fieldName ]
                );
                    ...
                    case ClassMetadata::MANY_TO_MANY:
                        $relation = ( new RelationManyToMany() )
                            ...
                            ->setMapInverseRelation(
                                $mapping['isOwningSide'] ? ( null !== $mapping['inversedBy'] ) : true
                            );
        ...
        if ( $classReflection ) {
            ...
            $targetFields = array_filter( $targetFields, function ( $field ) use ( $classReflection ) {
                return $classReflection->hasProperty( $field ) &&
                       $classReflection->getProperty( $field )->getDeclaringClass()->getName() ==
