    ...
    private function addDbalSection( ArrayNodeDefinition $node ): void
    {
        $node
            ...
            ->then( static function ( $v ) {
                ...
                $v['default_connection'] = isset( $v['default_connection'] ) ? (string)$v['default_connection']
                    : 'default';
            ...
            ->setDeprecated(
                ...
                $this->getDeprecationMsg(
                    'The doctrine-bundle type commenting features were removed; the corresponding config' .
                    ' parameter was deprecated in 2.0 and will be dropped in 3.0.',
                    '2.0'
                )
            )
            ->end()
    ...
    private function getDbalConnectionsNode(): ArrayNodeDefinition
    {
        ...
        $connectionNode
            ...
            ->setDeprecated(
                ...
                $this->getDeprecationMsg(
                    'The "keep_slave" configuration key is deprecated since doctrine-bundle 2.2. Use the ' .
                    '"keep_replica" configuration key instead.',
                    '2.2'
                )
            )
            ->end()
            ...
            ->info(
                sprintf(
                    'This option is used by the schema-tool and affects generated SQL. Possible keys include ' .
                    "'charset','%s', and 'engine'.",
                    $collationKey
                )
            )
            ->useAttributeAsKey( 'name' )
        ...
        $slaveNode = $connectionNode
            ...
            ->setDeprecated(
                ...
                $this->getDeprecationMsg(
                    'The "slaves" configuration key will be renamed to "replicas" in doctrine-bundle 3.0.' .
                    ' "slaves" is deprecated since doctrine-bundle 2.2.',
                    '2.2'
                )
            )
            ->useAttributeAsKey( 'name' )
    ...
    private function configureDbalDriverNode( ArrayNodeDefinition $node ): void
    {
        $node
            ...
            ->always( static function ( array $values ) {
                ...
                $urlConflictingOptions = [
                    'host'   => true, 'port' => true, 'user' => true, 'password' => true, 'path' => true,
                    'dbname' => true, 'unix_socket' => true, 'memory' => true,
                ];
                $urlConflictingValues  = array_keys( array_intersect_key( $values, $urlConflictingOptions ) );

                if ( $urlConflictingValues ) {
                    $tail = count( $urlConflictingValues ) > 1 ? sprintf(
                        'or "%s" options',
                        array_pop( $urlConflictingValues )
                    ) : 'option';
                    trigger_deprecation(
            ...
            ->children()
            ->scalarNode( 'url' )->info(
                'A URL with connection information; any parameter value parsed from this string will override ' .
            ...
            ?->booleanNode( 'override_url' )->setDeprecated(
                ...
                $this->getDeprecationMsg( 'The "doctrine.dbal.override_url" configuration key is deprecated.', '2.4' )
            )->end()
            ...
            ->info(
                ...
                'https://docs.oracle.com/database/121/NETAG/naming.htm. When using this option, you will still need' .
                ' to provide the user and password parameters, but the other parameters will no longer be used. ' .
                ' Note that when using this parameter, the getHost and getPort methods from Doctrine\DBAL\Connection ' .
    ...
    private function addOrmSection( ArrayNodeDefinition $node ): void
    {
        $node
            ...
            ->ifTrue( static function ( $v ) {
                ...
                return $v === null ||
                       ( is_array( $v ) &&
                         !array_key_exists( 'entity_managers', $v ) &&
            ...
            ->then( static function ( $v ) {
                ...
                foreach ( $v as $key => $value ) {
                    ...
                    $entityManager[ $key ] = $value;
                    unset( $v[ $key ] );
                }

                $v['default_entity_manager'] = isset( $v['default_entity_manager'] )
                    ? (string)$v['default_entity_manager'] : 'default';
            ...
            ->ifTrue( function ( $v ) {
                ...
                if ( is_int( $v ) && in_array( $v, $generationModes['values']/*array(0, 1, 2, 3)*/, true ) )
                    return false;
                ...
                if ( is_string( $v ) )
                    if ( in_array(
                        strtoupper( $v ),
                        /*array('NEVER', 'ALWAYS', 'FILE_NOT_EXISTS', 'EVAL', 'FILE_NOT_EXISTS_OR_CHANGED')*/
                        $generationModes['names'],
                        true
                    ) )
                        return false;
