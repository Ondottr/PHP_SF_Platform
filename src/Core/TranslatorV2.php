...
final class TranslatorV2 implements TranslatorInterface
{
    ...
    public function trans(string $id, array $parameters = [], ?string $domain = null, ?string $locale = null): string
    {
        ...
        if (null === $raw && DEFAULT_LOCALE !== $locale) {
    ...
    private function resolveAtRef(string $key, string $locale, int $depth): string
    {
        ...
        $raw = $this->catalogs[$locale][$key]
            ?? (DEFAULT_LOCALE !== $locale ? ($this->catalogs[DEFAULT_LOCALE][$key] ?? null) : null);
        ...
        foreach ($iterator as $file) {
            ...
            foreach ($properties as $property) {
                ...
                foreach ($attrs as $attr) {
                    $name = $attr->getName();
                    if (Column::class === $name || JoinColumn::class === $name || JoinColumns::class === $name) {
                        $hasMappedAttr = true;
                        break;
                    }
                }
