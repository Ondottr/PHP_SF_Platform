...
class Router
{
        ...
        if ($data->middleware !== null) {
            ...
            foreach ($data->middleware as $middleware) {
                ...
                if ($middlewareReflectionClass->implementsInterface(MiddlewareInterface::class) === false)
                    throw new RuntimeException("Middleware for route `$data->name` in class `$data->class` must implements " . MiddlewareInterface::class);
    ...
    #[NoReturn] private static function route(): void
    {
        static::setRequest();
    ...
    {
        static::$routeMethodResponse = ( self::$controller )->{static::$currentRoute->method}(...static::$routeParams);
    }

    #[NoReturn] protected static function sendRouteMethodResponse(): never
    {
    ...
    #[Pure] public static function getRouteLink(string $routeName): string
    {
